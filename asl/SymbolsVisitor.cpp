  ...
  if (ctx->paramsDef()) {
    for(uint i = 0; i < ctx->paramsDef()->parameter().size(); ++i) {
      ...
      if (Symbols.findInCurrentScope(ident)) {
        ...
        visit(ctx->paramsDef()->parameter(i)->type());
...
antlrcpp::Any SymbolsVisitor::visitType(AslParser::TypeContext *ctx) {
  ...
  if (ctx->INT()) {
    ...
    uint rows = stoi(ctx->INTVAL(0)->getText());
    uint cols = stoi(ctx->INTVAL(1)->getText());
    TypesMgr::TypeId t = Types.createMatrixTy(rows, cols, matrixType);
    putTypeDecor(ctx, t);
  }
  DEBUG_EXIT();
  return 0;
